  ...
  return (
    <div className='w-full h-16 md:h-20 flex items-center justify-between'>
      ...
      <Link className='flex items-center focus:outline-none' to='/'>
        <IKImage src='/logo.png' className='h-8 w-8 mr-6 rounded-[8px]' alt='Toni blog logo' />
        <span className='text-2xl font-bold text-gray-800 hover:text-blue-800'>ToniBlog</span>
      </Link>
      {/* MOBILE MENU */}
      <div className='flex flex-col md:hidden '>
        <div className='cursor-pointer text-4xl z-50' onClick={() => setOpen((prev) => !prev)}>
          {open ? (
      ...
      <div className='hidden md:flex md:items-center md:justify-center gap-8 xl:gap-12 font-medium'>
        {menus.map((menu, index) => (
          <NavLink key={`lg${index}`} to={menu.link} className='text-gray-800 hover:text-gray-600'>
            {menu.title}
          </NavLink>
        ))}
